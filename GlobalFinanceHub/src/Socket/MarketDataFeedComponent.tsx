...
const getUrl = async ({ token }: MarketDataFeedProps) => {
...
const decodeProfobuf = ({ buffer }: MarketDataFeedProps) => {
  if (!protobufRoot) {
...
function MarketDataFeed({ token }: MarketDataFeedProps) {
  const [isConnected, setIsConnected] = useState(false);
  const [feedData, setFeedData] = useState([]);

  // Establish WebSocket connection
  useEffect(() => {
    const connectWebSocket = async ({ token }: MarketDataFeedProps) => {
      try {
